...
const updateProp = (el, name, lastValue, nextValue, isSvg) => {
  ...
  } else if (name === "style" && el.nodeType !== 11) {
...
const createElement = (node, lifecycle, isSvg) => {
  const el =
    node.type === TEXT_NODE
      ...
      : node.type === FRAGMENT || node.type === COMPONENT
...
const createComponentVNode = (name, props, children, state) => {
  const component = {
    $$fn: name,
...
const setState = (node, newState) => {
  ...
  if (nextState !== node.state) {
    const nextNode = createComponentVNode(
      node.$$fn,
      node.props,
      node.props.children,
      nextState
    ...
    Object.assign(node.children[0], patched);
  }
};
...
export const Fragment = props => {
  createVNode("#fragment", props, props.children, null, null, null, FRAGMENT);
};
...
export const patch = (lastNode, nextNode, container) => {
  ...
  patchElement(container, lastNode.el, lastNode, nextNode, lifecycle);
...
export const h = function(name, props) {
  ...
  return typeof name === "function"
    ? createComponentVNode(name, props, children, state)
